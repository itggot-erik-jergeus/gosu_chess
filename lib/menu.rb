...
class MenuWindow < Gosu::Window
    ...
    def update
      if Gosu::button_down?(Gosu::MsLeft) then
        if check(@play.x_left,@play.x_right,@play.y_top,@play.y_bottom)
          GameWindow.new.show
          #Går till CHOOSE GAMEMODE
        elsif check(@rules.x_left,@rules.x_right,@rules.y_top,@rules.y_bottom)
          @background_image = Gosu::Image.new('./')
          #Går till RULES
        elsif check(@quit.x_left,@quit.x_right,@quit.y_top,@quit.y_bottom)
          close
        end
      end
    end
